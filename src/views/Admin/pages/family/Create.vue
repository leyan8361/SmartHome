...
.family-create-component
	...
	el-row(:span="24" type="flex" align="middle" justify="center")
		el-button(type="primary" @click="submitForm" :loading="isLoading") 创建家庭
	...
	existHandle(){
		...
		this.hasExisted(this.family.name).then(e=>{
			...
			if(e.success){
				...
				e.hasExisted && notice.warning('您的家庭名字已被人注册')
			}
		})
	}
	verify(){
		if(!this.family.name){
			notice.warning('请输入家庭名字')
			return false
		}
		if(this.family.name.length > 8){
			notice.warning('家庭名字过长哦')
			return false
		}
